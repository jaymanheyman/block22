...
const deleteParty = async (id) => {
  try {
    ...
    if (response.ok) {
      ...
      // You may want to perform additional actions after deleting the party
    } else {
  ...
  try {
    ...
    parties.forEach((party) => {
      ...
      detailsButton.addEventListener('click', async (event) => {
        ...
        await renderSinglePartyById(id, partyElement);
      });
      ...
      // re-render parties list
      const updatedParties = await getAllParties();
      await renderParties(updatedParties);  
      });
    });
  } catch (error) {
    console.error(error);
